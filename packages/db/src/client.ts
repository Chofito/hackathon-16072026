...
/** Cliente anon — respeta RLS. Seguro para lecturas publicas / lado cliente. */
export function createAnonClient(env: ClientEnv = {}): Db {
  const url = env.url ?? process.env.SUPABASE_URL
  const key = env.key ?? process.env.SUPABASE_ANON_KEY
  if (!url || !key) {
    throw new Error('Faltan SUPABASE_URL o SUPABASE_ANON_KEY para el cliente anon')
  }
  return createClient<Database>(url, key)
}
