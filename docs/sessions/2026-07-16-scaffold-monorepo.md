...
| Alcance del código | Templates con `TODO(dev-*)`; `@pgt/core` y `@pgt/db` sí implementados (son el contrato) | Implementar scrapers/ingest completos |
...
- **[../../packages/core](../../packages/core):** `@pgt/core` — tipos de dominio (camelCase), matching por capas (`matchCapture`, `normalizeName`) y parsing portable JSON-LD/sitemap (`parseProductFromHtml`, `parseSitemapUrls`). Reutilizable en Bun y Deno.
- **[../../packages/db](../../packages/db):** `@pgt/db` — cliente Supabase tipado (service/anon), `database.types.ts` (placeholder regenerable con `gen:types`) y query helpers con mapeo snake_case↔camelCase.
- **[../../packages/scrapers](../../packages/scrapers):** `@pgt/scrapers` — TEMPLATE: interfaz `Scraper`, fetch cortés, stub `max.ts`, registry. Restricción: runtime-agnóstico.
- **[../../packages/ingest](../../packages/ingest):** `@pgt/ingest` — TEMPLATE: glue matching → insert `price_points` → cola de revisión.
- **[../../apps/collector](../../apps/collector):** `@pgt/collector` — TEMPLATE de CLI que orquesta scrapers → ingest → Supabase (local).
...
- El autor instancia `apps/web` (Next.js App Router + Tailwind + shadcn/Radix, paquete `@pgt/web`).
