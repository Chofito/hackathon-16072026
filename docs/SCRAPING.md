...
| **MAX**      | max.com.gt                   | JSON-LD `Product` parcial ⚠️                | ❌                                              | `sku` ✅             | ausente en JSON-LD ⚠️                                | índice → `sitemap-0.xml` + 14 por categoría                      | Next.js headless sobre Magento (`backoffice.max.com.gt/media/catalog`) | **Cloudflare + CloudFront**                                                                                      | JSON-LD para precio + `__NEXT_DATA__` para stock |
...
- **GTIN/EAN: solo Curacao lo expone** (y no en JSON-LD, sino en la tabla de especificaciones del HTML, `data-th="GTIN"`, EAN-13). Las otras 3 no publican GTIN en ningún lado. Consecuencia: la capa 1 de matching (EAN exacto, §7) funciona para Curacao y sirve para *sembrar* `products.ean_gtin`, pero el matching general sigue arrancando en normalización marca+modelo + revisión manual. Todas dan identificadores por tienda (`sku`/`mpn`) útiles como `store_sku`.
...
- **Curacao (la mejor fuente):** JSON-LD limpio con `offers.price` (precio actual), `availability`, `priceValidUntil`, `seller: "LCO Guatemala Store"`, más un `price` de nivel raíz que es el precio de lista (ej. `4199` tachado vs `2797` de venta) → mapea a `listPrice`. Storefront Guatemala en GTQ, SSR, sitemap propio (`_gt_`, ~2,213 URLs con `lastmod` diario → crawl incremental). `sku` (`473…`), `productId` y `**GTIN` (EAN-13)** viven en el HTML fuera del JSON-LD; capturarlos aparte (`store_sku` y `eanGtin`).
- **MAX (más fricción — el autor configuró su WAF):** frontend Next.js headless servido tras Cloudflare + CloudFront. El JSON-LD trae `price`, `priceCurrency`, `sku`, `brand` pero **no** `availability`; el stock hay que sacarlo del `__NEXT_DATA__`. Con user-agent identificable respondió 200 sin challenge, pero es la tienda de mayor riesgo de bloqueo: máxima cortesía y fallo ruidoso ante 403/503.
- **Kemik (sin JSON-LD):** Next.js App Router, SSR; el precio está como microdata (`itemprop="price"`, `priceCurrency`, `sku`) dentro de un bloque `schema.org/Offer`, con disponibilidad vía OG `product:availability` (`instock`/`outofstock`) y precio también en `og:title`. Sin API pública. `listPrice` solo aparece en items con descuento. Requiere módulo HTML aislado por tienda. Ojo con el contador `anonymous_request_count` (ventana 300 s) y el `robots.txt` que banea bots por nombre: máxima cortesía, UA identificable, volumen bajo.
